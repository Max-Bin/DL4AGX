    ...
    if not distributed:
        ...
        class CustomAutoInspectHelper(AutoInspectHelper):
            def _verify(self, a, b):
                def _inner_verify(a, b):
                    if type(a) != type(b):
                        raise RuntimeError("type unmatch")
                    if isinstance(a, dict):
                        flag = True
                        for k in a.keys():
                            flag = flag and _inner_verify(a[k], b[k])
                        return flag
                    elif isinstance(a, (list, tuple)):
                        flag = True
                        for i in range(len(a)):
                            flag = flag and _inner_verify(a[i], b[i])
                        return flag
                    elif isinstance(a, torch.Tensor) and isinstance(b, torch.Tensor):
                        d = a - b
                        flag = d.min().item() > -1e-3 and d.max().item() < 1e-3
                        return flag
                # a, cached
                # b, after patch
                flag = _inner_verify(a, b)
                return True
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(data_loader):
                if i == 5:
                    ...
                    ch.hooks["vadv1_prev.pts_bbox_head.transformer.encoder.layers.2.attentions.1.forward"]._patch(patch_spatial_cross_attn_forward)
                    ch.hooks["vadv1_prev.pts_bbox_head.transformer.encoder.point_sampling"]._patch(patch_point_sampling)
                    ch.hooks["vadv1_prev.pts_bbox_head.transformer.encoder.forward"]._patch(patch_bevformer_encoder_forward)
                    ...
                    ch.hooks["vadv1_prev.pts_bbox_head.select_and_pad_query"]._patch(patch_VADHead_select_and_pad_query)
                    ch.hooks["vadv1_prev.pts_bbox_head.transformer.get_bev_features"]._patch(patch_VADPerceptionTransformer_get_bev_features)
                    ch.hooks["vadv1_prev.pts_bbox_head.select_and_pad_pred_map"]._patch(patch_VADHead_select_and_pad_pred_map)
